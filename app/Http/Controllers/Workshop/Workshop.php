...
class Workshop extends Controller
{
    ...
    { 
        ...
        $finalworkdate= implode(",",$workshopdates);
        $trainer = TrainerModel::all();
        $attendence=AttendenceModel::where('type','workshop')->get();
        foreach($attendence as $row)
        {
        $workshopattendence = WorkshopModel::where('id',$row->class_id)->get();
        foreach($workshopattendence as $row1)
        {
        $getdate= $row1->workshopdates;
        $row['workshopTitle']=$row1->title;
        $datewise = json_decode($getdate,true);
        $getdatevl= array_search($row->date,$datewise,true);
        if($getdatevl != '')
        {
         $datewiseattendence=AttendenceModel::where('date',$row->date)->get(); 
         foreach($datewiseattendence as $getatt)
         {
    
            $student=StudentsModel::where('id',$getatt->student_id)->get();
            
            foreach($student as $std)
            {
                $row['studentName'] =$std->firstname . " " . $std->middlename . " " . $std->lastname;
        ...
        return view('content.classSchedule.singleWorkshopSchedule')->with('getWorkshop',$getWorkshop)->with('projectClass',$projectClass)->with('finalworkdate',$finalworkdate)->with('branch',$branch)->with('trainer',$trainer)->with('attendence',$attendence); 
